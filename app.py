...
st.markdown("### A professional-grade image editor by **Chetan**. Your all-in-one solution for stunning photos.")
...
if uploaded_file is not None:
    ...
    with st.sidebar.expander("🎨 Color Controls"):
        st.subheader("Color Balance")
        red_balance = st.slider("Red Channel", -50, 50, 0)
        green_balance = st.slider("Green Channel", -50, 50, 0)
        blue_balance = st.slider("Blue Channel", -50, 50, 0)
        st.subheader("Temperature")
        temp = st.slider("Warmth / Coolness", -100, 100, 0)
